...
| **signal** | A call-setup message — offer, answer, bye, ice — carried on the drive socket. `call_transport.js`, `drive.py`. **Claimed 2026-09-01.** The one message on that socket that travels BOTH ways. | A verb. A signal is never turned into a verb or a bus topic; that separation is the whole reason it is allowed to travel both ways. Also not the general "signal" of a bus publish — say *publish* for that. |
...
| **data link** | The third bus, named. A CONTINUOUS reference from one module instance's port to another's — the spreadsheet-cell-reference sense: one module's number, text, time or picture appears as another module's property, and keeps showing it. Not one thing running after another; one thing SHOWING another. Structural — it enters the composition fingerprint the same way a prefab override does — and carries endpoints only, `{from: (instance, port), to: (instance, port)}`; the running VALUE that flows over it carries the timestamp, the provenance and whether it is event or continuous, and those live on the message, not the link. See `bus_and_generator_20260910.md` §1.3-1.5. **Claimed 2026-09-10.** We already have one and it has no record: Trivia and Word Forge both read the same person's bank (`bank.js`), continuously, as a property of the game — that IS a data link, drawn by hand in two files with nothing to point at it or check it, which is exactly why nothing could until the word existed. | A "link" in `drive.js` / `call_transport.js` — an established, different sense: the signaling connection between two DEVICES. The two do not collide in one file today, but both are architecturally real, so say "data link" in full wherever the distinction matters, never bare "link" for this sense. |
| **port** | A named point on a module instance where a data link can attach — declared on the manifest, carrying its own type, direction (in/out) and class (event or continuous). The physical-connector analogue: a port's type and direction map one-to-one onto a real connector's kind and sex, the way `bus_and_generator_20260910.md` §1.3 argues them onto the message. **Claimed 2026-09-10.** | The network listening port (`--port 8000` in `app.py`) — a different, non-colliding sense that never appears near a module manifest. |
